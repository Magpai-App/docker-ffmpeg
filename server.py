            ...
            with TemporaryDirectory() as tmpdir:
                ...
                with Popen(
                    ...
                    if ffmpeg_process.returncode != 0:
                        logger.warning(f"Error running ffmpeg: {ffmpeg_process.returncode}")
                        self.send_response(500)
                        self.end_headers()
                        self.wfile.write(ffmpeg_stderr)
                        return

                    with (output_file_path := (tmp_path / "output")).open("rb") as output_file:
                        self.send_response(200)
                        self.send_header("Content-Type", "application/octet-stream")
                        self.send_header("Content-Length", str(output_file_path.stat().st_size))
                        self.end_headers()
...
def main(args: list[str]) -> int:
    argparser = ArgumentParser(description="FFmpeg Server")
    ...
    print(f"Starting FFmpeg Server on {args.host}:{args.port}...", flush=True)
    server_thread.join()
    return 0
